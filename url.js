...
// 設定最低分配數量和總回應數 const minCount = 20; const totalResponses = 600; // 假設總共需要 600 份問卷 let distribution = {}; let finalList = [];
// 1. 初始化，每份問卷先分配 20 份 urls.forEach(url => { distribution[url] = minCount; for (let i = 0; i < minCount; i++) { finalList.push(url); } });
// 2. 剩餘數量，確保所有問卷均達標後才開始分配 let remaining = totalResponses - (minCount * urls.length); let availableUrls = […urls]; // 建立一個可分配額外數量的網址池
while (remaining > 0) { let randomIndex = Math.floor(Math.random() * availableUrls.length); let selectedUrl = availableUrls[randomIndex];
distribution[selectedUrl]++;
finalList.push(selectedUrl);

remaining--;
}
// 3. 最後使用 Fisher-Yates 洗牌演算法隨機排列問卷 function shuffleArray(array) { for (let i = array.length - 1; i > 0; i–) { const j = Math.floor(Math.random() * (i + 1)); [array[i], array[j]] = [array[j], array[i]]; } } shuffleArray(finalList);
// 4. 輸出結果 console.log(“隨機分配後的 600 份問卷清單：”, finalList); console.log(“最終分配統計：”, distribution);
// 5. 提供隨機問卷連結函數（用於網頁應用） function getRandomFormLink() { return finalList.pop(); // 取出一個問卷，確保不會重複給到 }
